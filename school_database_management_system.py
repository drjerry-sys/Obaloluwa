...
class SDBM:
    ...
    def noticeboard_or_reportSend(self):
        self.dbconnection()
        self.present = str(datetime.datetime.now())
        self.heading_get, self.body_get, self.summary_get, self.postingDate_get\
            = self.reportheading.get(), self.reportbody.get(1.0,END), self.reportsum.get(1.0, END), self.postingDate.get()
        self.postingDate_get = str(self.postingDate_get).replace('/', '-')
        if self.report_or_Nboard == 'n':
            if len(self.heading_get) <=34 and len(self.body_get)<=149 and len(self.summary_get)<=63:
                self.mycursor.execute('INSERT INTO noticeboard(Heading, Body, Date_Updated, Time_Updated, Summary, Staff_aid, Post_Date) VALUES(%s,%s,%s,%s,%s,%s,%s)',
                (self.heading_get, self.body_get, self.present[:10], self.present[11:16], self.summary_get, self.invarid.get(), self.postingDate_get))
                self.mycon.commit()
                showinfo('PGS', 'Notice Board Successfully Updated!')
                self.writereport('n')
            else:
                showinfo('Too Much Info', 'Max Heading, Body, Summary\n should be 34, 149, 63 characters respectively')
        elif self.report_or_Nboard == 'r':
            if len(self.heading_get) <=34 and len(self.body_get)<=149 and len(self.summary_get)<=63:
                self.mycursor.execute('INSERT INTO report_tb(Heading, Body, Date_Updated, Time_Updated, Summary, Staff_aid) VALUES(%s,%s,%s,%s,%s,%s)',
                                  (self.heading_get, self.body_get, self.present[:10], self.present[11:16], self.summary_get, self.invarid.get()))
                self.mycon.commit()
                showinfo('PGS', 'Report Sent Successfully!')
                self.writereport('r')
            else:
                showinfo('Too Much Info', 'Max Heading, Body, Summary\n should be 34, 149, 63 characters respectively')
    ...
    def noticeboardupdate(self, course_s):
        ...
        if course_s == 0:
            ...
            Label(self.temporal, text='STUDENTS\' COURSES', width=37, height=2, bg='#3E4095', fg='#ffffff').place(x=1, y=0)
            Label(self.temporal, text='UPDATE COURSES', bg='#3E4095', fg='#ffffff', width=37, height=1).place(x=1, y=37)
            self.writepad = Text(self.temporal, width=33, height=28, bg='#E6E7E8')
            self.writepad.place(x=0, y=58)
            Button(self.temporal, text='OK ', bg='#3E4095', width=22, fg='#ffffff', bd=0, cursor='hand2', command=lambda:self.ok('1', 1)).place(x=0, y=506)
            Button(self.temporal, text='BACK ', bg='red', width=15, fg='#ffffff', bd=0, cursor='hand2', command=lambda:self.noticeboard(1,1)).place(x=160, y=506)
    ...
    def staff_reg(self):
        self.release = 0
        likely = {'Email':self.invaremail, 'Phone':self.invarphone, 'Passport':self.invarpas}
        for key, value in likely.items():
            if len(value.get()) == 0:
                showerror('Incomplete Info', f'You Must fill the {key} field')
                break
            elif (value == self.invaremail) and ('.' and '@' not in self.invaremail.get()): # to confirm if the email has . and @
                showerror('Wrong Info', 'Specified Email is not correct')
                break
            else: # if none of the next line condition is not fufilled, the process breaks at the loop
                if (self.invaremail.get() and self.invarphone.get() and self.invarpas.get() != '') and ('.' and '@' in self.invaremail.get()):
                    self.release = 1
        if self.release == 1:
            shutil.copy(self.in_path, 'C:\\Users\\OBALOLUWA\\Pictures\\camcam')
            # save the new path into the database, the new path now is:
            self.passport = os.path.basename(self.in_path)
            self.passSave = "C:\\Users\\OBALOLUWA\\Pictures\\camcam\\" + self.passport
            self.admnstudent('3')
            self.query = 'INSERT INTO staffs_tb(First_name, Middle_name, Last_name, Phone_number, Email, Address, Passport, Password) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)'
            self.save = (self.invarsname.get(), self.invarfname.get(), self.invarlname.get(), self.invarphone.get(), self.invaremail.get(), self.invaradres.get(), self.passSave, self.invarsname.get())
            self.mycursor.execute(self.query, self.save)
            self.mycon.commit()
            # back_end assigning and displaying the real staff_id
            self.mycursor.execute('SELECT staff_id FROM staff_tb WHERE lower(Email) = %s', (self.invaremail.get().casefold(),))
            self.myfetch = self.mycursor.fetchone()
            if self.invartitle.get().casefold() == 'teacher':
                if len(self.myfetch[0]) == 1:
                    self.realstaff_id = 'PGSTF' + '/00' + self.myfetch[0]
                elif len(self.myfetch[0]) == 2:
                    self.realstaff_id = 'PGSTF' + '/0' + self.myfetch[0]
                elif len(self.myfetch[0]) == 3:
                    self.realstaff_id = 'PGSTF' + '/' + self.myfetch[0]
            elif self.invartitle.get().casefold() == 'admin':
                if len(self.myfetch[0]) == 1:
                    self.realstaff_id = 'PGADM' + '/0' + self.myfetch[0]
                elif len(self.myfetch[0]) == 2:
                    self.realstaff_id = 'PGADM' + '/' + self.myfetch[0]
            self.mycursor.execute(f'UPDATE staffs_tb SET Staff_aid = %s',(self.realstaff_id,))
            self.mycon.commit()
            showinfo('PGS ID-Generator', f'staff ID is {self.realstaff_id}\n Password is the Surname')
...
class CbtExam(SDBM):
    ...
    def enterExamPage(self):
        Frame(self.destroyFrame, width=800, height=40, bg='#606062').place(x=251, y=0)
        Label(self.root, text='P r a c t i s i n g  G r a m m a r  S c h o o l', font=('Arial', 8, 'bold'),
              fg='#ffffff', bg='#606062').place(x=12, y=0)
        self.logo = Image.open('images/fooa.png')
        self.schl = ImageTk.PhotoImage(self.logo)
        Label(self.exalogoframe, image=self.schl, bg='#606062').place(x=90, y=60)
        Label(self.exalogoframe, text='your time ticks!', bg='#606062', fg='#ffffff').place(x=65,y=280)
        super().dbconnection()
        self.mycursor.execute('SELECT course_id, course_name FROM course_tb WHERE class_id = %s', (SDBM.class_id,))
        self.myfetch = self.mycursor.fetchall()
        self.courseDict, self.courseValue = dict(), []
        for course in self.myfetch:
            self.courseDict[course[1]] = course[0]
            self.courseValue.append(course[1])
        self.student_id = SDBM.student_id
        Button(self.exalogoframe, text='Exit', bg='darkred', fg='#ffffff', bd=0, width=10, command=self.exit).place(x=31, y=400)
        Button(self.exalogoframe, text='Submit', bg='darkgreen', fg='#ffffff', bd=0, width=15, command=self.examTestSubmit).place(x=101, y=400)
        Button(self.exalogoframe, text='Calculator', bg='#C8C5E2', fg='#000000', bd=0, width=25,
               command=self.calculator).place(x=31, y=423)
        self.timedisp = Label(self.exalogoframe, text='00:00:00', fg='#ffffff', bg='#606062', font=('Arial', 30, 'bold'))
        self.timedisp.place(x=31, y=300)
        self.mycursor.execute('SELECT Control_Value FROM control_tb WHERE control_id = %s', (2,))
        self.myfetch = self.mycursor.fetchone()[0]
        if self.myfetch == '1':
            Label(self.root, text='TEST', fg='#ffffff', bg='#606062', font=('Arial', 12, 'bold')).place(x=257, y=7)
        elif self.myfetch == '2':
            Label(self.root, text='EXAMINATION', fg='#ffffff', bg='#606062', font=('Arial', 10, 'bold')).place(x=257,y=7)
    ...
    def firstSeen(self):
        super().dbconnection()
        self.present = str(datetime.datetime.now())
        self.mycursor.execute('SELECT class_id, course_id, course_name FROM course_tb JOIN student_tb USING\
        ...
        self.logo = Image.open('images/fooa.png')
        self.schl = ImageTk.PhotoImage(self.logo)
        self.countdown = Label(self.destroyFrame, image=self.schl, bg='#606062', text='PGS', font=('Arial', 20, 'bold'), fg='#ffffff',compound='top')
        self.countdown.place(x=500, y=220)
        if self.myfetchinfo and self.levels == 0:
            self.timer(1)
        else:
            if self.levels == 0:
                showinfo('PGS', 'You do not Have any Exam at this particular time')
            else:
                showinfo('PGS', 'Good-bye!\n You\'re done.')
            self.root.quit()
